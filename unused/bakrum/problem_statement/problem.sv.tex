...
Mer exakt finns det ett $R \times C$ stort mönster som repeterar sig oändligt.
Se bilden nedan för exakt hur detta funkar.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.4\textwidth]{sampleimage.pdf}
  \caption{Visualisering av exempelfall 1. Grundmönstret är markerat i rött och koordinaterna av den första frågan är ritade vid $(2,4)$ och $(4,3)$.
  Självfallet fortsätter rutnätet oändligt bortom det som ritats.}
\end{figure}
...
Därefter följer fem rader.
Varje av dessa rader innehåller fyra heltal $s_x, s_y, g_x, g_y$ ($0 \leq s_x, s_y, g_x, g_y \leq 10^{12}$), koordinaterna på en fråga.
Det är garanterat att ingen av rutorna vid dessa koordinater är blockerade.
...
\begin{itemize}
  \item Fråga 1: Harry börjar på $(2,4)$ och vill flytta sig till $(4,3)$.  
  För att göra detta kan han röra sig höger, ner och höger.
  \item Fråga 2: start- och målpositionerna separeras av väggar, så svaret är ``\texttt{Nej}''.
  \item Fråga 3: Harry börjar redan på målet, så han är klar direkt.
  \item Fråga 4: även om vi befinner oss långt bort fortsätter rutnätet. Svaret blir ``\texttt{Ja}''.
  \item Fråga 5: återigen separeras start- och målpositionerna av väggar.
\end{itemize}
