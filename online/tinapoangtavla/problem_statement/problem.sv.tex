...
ICPC är en programmeringstävling, där man ska försöka lösa så många problem som möjligt på fem timmar.
...
Först räknar man hur många problem som varje lag löst, låt detta vara $p$.
Sedan beräknar man varje lags \textit{inskick-straff}.
För varje problem där ett lag får Accepted är inskick-straff lika med antal felaktiga inskick laget gjorde på problemet.
Inskick-straffet för ett lag är summan av inskick-straff över alla problem de löste, vilket vi betecknar med $t$.
Varje lags resultat kan därmed beskrivas med talen $(p,t)$.
Notera att inskick på problem som ett lag inte löser under hela tävlingen inte ökar tidsstraffet.
Rangen av ett visst lag är antalet lag som är \textit{strikt bättre}.
Ett lag $i$ är strikt bättre om lag $j$ om antingen
...
De första fyra timmarna får varje lag se vare sig varje av deras inskick fick Accepted eller inte genom Kattis.
De kan även se resultaten av andra lags inskick genom poängtavlan.
Den sista timmen blir poängtavlan \textit{fryst}, och därmed lagen endast se resultatet av sina egna inskick.
Varje lag kan fortfarande se när nya inskick sker av andra lag, men inte deras resultat.

Efter tävlingen är över är det dags att \textit{tina} poängtavlan.
...
Om problemen istället hade avslöjats i en ordning så att det tar längre tid för de att få reda på detta blir det självfallet mer spännande.
...
($1 \leq w \leq N$, $1 \leq p \leq P$, $r \in \{A,W\}$),
som beskriver att laget med index $w$ skickade in till problem $p$,
vars resultat var $r$. Om $r$ är A betyder det att inskicket fick Accepted, annars fick det inte det.
