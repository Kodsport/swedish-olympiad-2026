...
int main()
{
    ...
    auto compute_breakpoint = [&](int dungeon_index, ll to_beat)
    {
        ll start = 1e10 + 1;
        ll lo = -1;
        ll hi = start;
        ...
        {
            ll mid = (lo + hi) / 2;
            if (simulate_dungeon(dungeon_index, mid).first >= to_beat) hi = mid;
            else lo = mid;
        }
    ...
    rep(i, n) // i just unlocked dungeon i
    {
        ...
        auto cost_to_win = [&]()
        {
            ...
            if (best_prev)
            {
                ll breakeven_strength = compute_breakpoint(i, best_prev);
            ...
            while (str < b)
            {
                ll curr_gain = simulate_dungeon(i, str).first;
                ll new_strength = compute_breakpoint(i, curr_gain + 1);
                if (new_strength == -inf) // no improvement possible
                {
                    new_strength = b;
                }
                new_strength = min(new_strength, (ll)b);
                ll num_runs = (new_strength - str + curr_gain - 1) / curr_gain;
        ...
        {
            ll curr_gain = simulate_dungeon(i, strength).first;
            ll new_strength = compute_breakpoint(i, curr_gain + 1);
            if (new_strength == -inf) // no improvement possible
            {
                new_strength = b;
            }
            new_strength = min(new_strength, lo_strength[i]);
            ll num_runs = (new_strength - strength + curr_gain - 1) / curr_gain;
            strength += num_runs * curr_gain;
            runs += num_runs;
        }
        runs++;
