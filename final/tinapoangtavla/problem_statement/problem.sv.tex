...
Detta problem handlar om en tävling som är lik, men inte exakt samma som officiella ICPC.

I denna tävling gör lag inskick på problem.
För varje inskick får man inte poäng, utan bara \textit{Accepted} eller \textit{Wrong Answer}.
I slutet av tävlingen etablerar man en rangordning av lag på följande vis.
Först räknar man hur många problem som varje lag löst, låt detta betecknas av ett heltal $p$.
...
Inskick-straffet för ett lag är summan av inskick-straff över alla problem \underline{de löste}, vilket vi betecknar med ett heltal $t$.
Varje lags resultat kan därmed beskrivas med talparet $(p,t)$.
Notera att om ett lag aldrig löser ett visst problem under hela tävlingen, så får de inget inskicks-straff från det problemet.
...
De första fyra timmarna under tävlingen får varje lag se om deras inskick blir Accepted eller inte.
...
Tina har som målsättning att göra ICPC mer spännande.
Hon har därför föreslagit en ordning på hur resultat av inskick ska avslöjas.
Givet denna ordning vill hon för varje lag beräkna antalet inskick som avslöjas innan de tappar intresset.
...
Tina vill då veta för varje lag, antalet inskickningar som avslöjas innan de tappade intresset.
...
\section*{Indata}
Den första raden innehåller fyra heltal $N$, $P$, $H$ och $F$
($1 \leq N \leq 2 \cdot 10^5$, $1 \leq P \leq 15$, $0 \leq H \leq 2 \cdot 10^5$, $1 \leq F \leq 2 \cdot 10^5$),
antalet lag, antalet problem, antalet inskick före frysen och antalet inskick under frysen.

De följande $H$ raderna beskriver inskicken som skedde före frysen.
Varje sådant inskick beskrivs av heltalen $w$ och $p$ samt strängen $r$
($1 \leq w \leq N$, $1 \leq p \leq P$, $r \in \{AC,WA\}$),
som beskriver att laget med index $w$ skickade in till problem $p$,
vars resultat var $r$. Om $r$ är AC betyder det att inskicket fick Accepted, annars fick det inte det.
Dessa inskick är ordnade i kronologisk ordning, det vill säga är dessa inskickningar sorterade på $t$.

De följande $F$ raderna beskriver inskicken som skedde under frysen.
Formatet är identiskt med inskicken före frysen, men dessa är givna i den ordning som Tina föreslagit för avslöjandet av resultaten och är inte nödvändigtvis kronologiskt sorterade.
Det är dock garanterat att för varje lag och varje problem kommer lagets inskick i kronologisk ordning.
...
\section*{Utdata}
För varje lag, skriv ut antalet inskickningar det tog innan laget tappade intresset.
...
% # easier correctness-wise:
% H=0
% varje lag gör som mest ett inskick per problem.
% P=1
...
\begin{tabular}{| l | l | p{12cm} |}
  ...
  $1$    & $5$        & $N \leq 50$, $P=1$ och varje lag gör som mest ett inskick per problem. \\ \hline
  $2$    & $6$        & $N \leq 50$, och varje lag gör som mest ett inskick per problem. \\ \hline
  $3$    & $15$       & $N,F,H \leq 50$ \\ \hline
  $4$    & $8$        & $N,F,H \leq 500$ \\ \hline
  $5$    & $30$       & $N,F,H \leq 2000$ \\ \hline
  $6$    & $36$       & Inga ytterligare begränsningar. \\ \hline
\end{tabular}
...
Lag $1$ vet inte om lag $2$ kommer lösa problemet eller inte, men även om det andra laget löser problemet,
...
När resultatet väl avslöjats blir de säkra.

Exempelfall 3: även om det inte verkar så kan lag $1$ lista ut direkt att de har vunnit tävlingen.
