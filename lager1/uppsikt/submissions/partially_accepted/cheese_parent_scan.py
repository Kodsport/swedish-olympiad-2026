...
#
# Cheese: Parent-finding with early break. O(n^2) worst case on sorted data,
# but O(n log n) expected on random (nearest taller element is close).
...
def build_cartesian_tree(p):
    ...
    for i in range(n):
        ...
        if left_greater == -1:
            parent[i] = right_greater
        elif right_greater == -1:
            parent[i] = left_greater
        elif p[left_greater] < p[right_greater]:
            parent[i] = left_greater
        else:
            parent[i] = right_greater
