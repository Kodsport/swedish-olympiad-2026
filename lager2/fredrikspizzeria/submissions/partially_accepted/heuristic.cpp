...
int main() {
    ...
    // DFS to build Block-Cut Tree components
    while (!st.empty()) {
        Frame& cur = st.back();
        int u = cur.u;
    ...
    // Evaluate 1 -> 2 escape constraints
    ...
    // Generate requirement signatures for cutting specific variables
    for (int v : vars) {
        Signature cur;
    ...
    // Cycle check with heuristic pruning
    ...
    for (int cid : cycles13) {
        ...
        int operations = 0;
        const int MAX_OPS = 2500; // Heuristic exit limit
        ...
        for (int u : a_vars) {
            ...
            for (int v : b_vars) {
                ...
                if (operations > MAX_OPS) break; // Prune

                // Check signature compatibility naively
                bool conflict = false;
                for (const auto& req_u : sig[u].req) {
                    for (const auto& req_v : sig[v].req) {
