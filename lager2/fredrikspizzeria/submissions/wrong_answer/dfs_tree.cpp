...
static uint64_t single_key(int cycle, int value) { return (uint64_t(cycle) << 1) | uint64_t(value); }
static uint64_t pair_key(uint64_t a, uint64_t b) { if (a > b) swap(a, b); return (a << 32) ^ b; }
...
int main() {
    ...
    vector<int> cycle_backedge; // Track back-edges explicitly
    cycle_backedge.push_back(0); 
    ...
    while (!st.empty()) {
        ...
        if (state[v] == 0) {
            ...
            cycle_backedge.push_back(eid); // FAKE SOLVE: Record the back-edge
    ...
    for (int x = 2; x != 1; x = parent_node[x]) {
        ...
        if (cycle_id[e] != 0 && !mark_cycle12[cycle_id[e]]) {
            mark_cycle12[cycle_id[e]] = 1; cycles12.push_back(cycle_id[e]);
        }
    }
    ...
    for (int x = 3; x != 1; x = parent_node[x]) {
        ...
        if (cycle_id[e] == 0) {
            ...
            mark_cycle13[cycle_id[e]] = 1; cycles13.push_back(cycle_id[e]);
        }
    }
    ...
    // FAKE SOLVE LOGIC: Hardcode branches based purely on DFS tree topology
    for (int cid : cycles13) {
        for (int e : cycle_edges[cid]) {
            branch13[e] = (e == cycle_backedge[cid]) ? 0 : 1; 
        }
    }
    ...
    for (int v : vars) {
        ...
        for (int e : var_edges[v]) {
            if (in12[e] && cycle_id[e] == 0) { cur.ok = false; break; }
            if (branch12[e] != -1) {
    ...
    // Remaining collision mapping logic is functionally identical to the exact solution
    // but operates on the corrupted branch13 definitions.
    for (int cid : cycles13) {
        ...
        for (int v : b_vars) {
            ...
            ++total_b;
            if (sig[v].req.size() >= 1) ++bad_single_b[single_key(sig[v].req[0].cycle, sig[v].req[0].value)];
            if (sig[v].req.size() == 2) {
                ++bad_single_b[single_key(sig[v].req[1].cycle, sig[v].req[1].value)];
                ++bad_double_b[pair_key(single_key(sig[v].req[0].cycle, sig[v].req[0].value), single_key(sig[v].req[1].cycle, sig[v].req[1].value))];
            }
        }
        ...
        for (int v : a_vars) {
            ...
            if (req.empty()) { if (total_b > 0) { cout << "Ja\n"; return 0; } continue; }
