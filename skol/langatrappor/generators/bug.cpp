...
int main()
{
	...
	auto cost_segment = [&](ll a, ll b, ll i) {
		if (a > b) return 0LL;
		ll n = b - a + 1;
		ll sum_j = (a + b) * n / 2;
		return n * start_val[i] - sum_j;
	};
	...
	rep(i, c)
	{
		...
		if (start.first != goal.first)
		{
			cost = col_dist(i, startcol, start.first) + col_dist(i, goalcol, goal.first);
		}
		else cost = col_dist(startcol, goalcol, start.first);

		cost += cost_segment(goal.first + 1, start.first-1, i);
